...
private object AuthSQL {
  ...
  def insert[A](jwt: AugmentedJWT[A, Long])(implicit hs: JWSSerializer[JWSMacHeader[A]]): Update0 =
    sql"""INSERT INTO JWT (jwt_id, JWT, account_id, EXPIRY, LAST_TOUCHED)
  ...
  def update[A](jwt: AugmentedJWT[A, Long])(implicit hs: JWSSerializer[JWSMacHeader[A]]): Update0 =
    sql"""UPDATE JWT SET JWT = ${jwt.jwt.toEncodedString}, account_id = ${jwt.identity},
         | EXPIRY = ${jwt.expiry}, LAST_TOUCHED = ${jwt.lastTouched} WHERE jwt_id = ${jwt.id}
       """.stripMargin.update

  def delete(id: SecureRandomId): Update0 =
    sql"DELETE FROM JWT WHERE jwt_id = $id".update

  def select(id: SecureRandomId): Query0[(String, Long, Instant, Option[Instant])] =
    sql"SELECT JWT, account_id, EXPIRY, LAST_TOUCHED FROM JWT WHERE jwt_id = $id"
